...
**MyBatis-Plus 版（选项三）**：目录结构相同，但 XML 映射文件仅在主子表或自定义 SQL 时需要。
...
public interface I{ClassName}Service extends IService<{ClassName}> {
    ...
    // 新增（返回 boolean，与 MP save() 一致）
    boolean insert{ClassName}({ClassName} {className});
    
    // 修改（返回 boolean，与 MP updateById() 一致）
    boolean update{ClassName}({ClassName} {className});
    
    // 删除单条
    boolean delete{ClassName}By{PkField}({PkType} {pkField});
    
    // 批量删除
    boolean delete{ClassName}By{PkField}s({PkType}[] {pkField}s);
}
```

> **注意**：选项三 Controller 调用 `toAjax(boolean)` 适配 MP 的 boolean 返回值。需要 `BaseController` 包含 `toAjax(boolean result)` 重载（RuoYi-Vue-Plus 已内置）。

### 5.3 Mapper XML 规范

> **MyBatis-Plus（选项三）**：标准 CRUD 操作无需 XML 映射文件（由 `BaseMapper` 提供实现）。仅子表批量操作、复杂自定义查询需要 XML。
...
## 八、MyBatis-Plus 特定规范（选项三）

### 8.1 架构分层

```
Controller → 调用 Service 业务方法（不直接接触 MP API）
     │
Service 接口 → extends IService<T> + 声明业务方法（返回 boolean）
     │
ServiceImpl → extends ServiceImpl<M, T>
     │         ├── super.save()          → 新增
     │         ├── super.updateById()    → 修改
     ...
     │
Mapper → extends BaseMapper<T>（无需声明 CRUD 方法）
```

### 8.2 禁止事项

- ❌ Controller 中直接使用 `LambdaQueryWrapper` 或 MP API
- ❌ ServiceImpl 中调用 `baseMapper.insert/updateById/deleteById` 等标准 CRUD（应使用 `super.xxx()`）
- ❌ 在 Mapper 中重复声明 `BaseMapper` 已有的 CRUD 方法

### 8.3 允许事项

- ✅ 子表批量操作、复杂自定义查询通过 `this.baseMapper.customMethod()` 调用（这些方法需在 Mapper 接口声明 + XML 实现）
- ✅ `selectXxxList` 中使用 `LambdaQueryWrapper` 构建动态查询条件

### 8.4 前置要求

| 要求 | 说明 |
|------|------|
| `mybatis-plus-boot-starter` | 需引入 MP 依赖 |
| `PaginationInnerInterceptor` | 替换 `PageHelper`，配置 MP 分页插件 |
| `@MapperScan` 或 `@Mapper` | Mapper 接口需注册（同标准 MyBatis） |
| `toAjax(boolean)` | `BaseController` 需包含 boolean 重载（RuoYi-Vue-Plus 已内置） |

### 8.5 ServiceImpl 实现速查

| 业务方法 | MP 实现 | 返回值 |
|---------|--------|--------|
| `selectXxxById(id)` | `this.getById(id)` | 实体对象 |
| `selectXxxList(xxx)` | `LambdaQueryWrapper` + `this.list(wrapper)` | `List<T>` |
| `insertXxx(xxx)` | `super.save(xxx)`（回填主键） | `boolean` |
...
| 子表操作 | `this.baseMapper.batchXxx()` / `deleteXxxByXxx()` | `int`（走 XML） |
